...
{
    [Header("배회")]
    public float patrolRadius = 3f;    // 배회 가능 반경
    public float moveSpeed = 2f;       // 이동 속도
    public float idleTime = 2f;        // 멈춰있는 시간

    [Header("추적")]
    public float detectRange = 5f;    // 플레이어를 감지할 범위
    ...
    IEnumerator ChaseRoutine()
    {
        ...
        while (player != null && Vector3.Distance(transform.position, player.position) <= detectRange + 2f) // 놓치는 범위는 감지보다 조금 더 넓게 설정(히스테리시스)
        {
            // 플레이어 방향으로 이동
    ...
    IEnumerator PatrolRoutine()
    {
        ...
        {
            if (IsPlayerDetected()) yield break; // 대기 중 플레이어 발견 시 즉시 종료
